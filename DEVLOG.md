...
- **User report on release day:** his Windows terminal chats from 2026-07-09 had not reached the Mac by themselves, and `memex sync status` on the Mac said "Auto-sync: off" despite `setup --sync` (2026-07-04). The diagnosis separated three suspects:
  - **[fixed] `sync status` rendered only the `MEMEX_SYNC_AUTO` env var**, while the serve lifespan starts the loop on env OR the persisted gate OR a configured file-sync dir. The Mac's gate file has `sync_auto: true`, so the loop was configured ON while status said off. Fix: new `sync_state.auto_sync_effective()` is the single definition of the condition and BOTH the lifespan and `status` call it, so they can never drift again. +2 regression tests (helper reads all three sources; status flips with the persisted gate). **685 green**, ruff + format + CI mypy scope clean. UNCOMMITTED pending the user's review.
  - **[verified live, not a bug] The Mac's auto-sync loop IS alive.** With no manual sync running, the `windows` breadcrumb in `sync_history.json` advanced on its own at 15:32:49 UTC (serve up since 2026-07-09 23:34 local, interval 900 s; observed by polling the file). Diagnosability note: the loop's INFO lines (including the startup "auto-sync enabled") never reach `serve.log` because the app module's logging is unconfigured under uvicorn, so the log cannot answer "is the loop on"; not fixed today, worth a future look.
  - **[real root cause, Windows-side, OPEN] The 2026-07-09 chats were never INDEXED on Windows automatically.** The moment the user indexed them by hand (MCP `index_terminal_sessions` from the Windows Claude), sync moved them immediately (the Windows-initiated reconcile is visible in the Mac's serve.log, and a Mac `sync now` had already pushed this session to Windows). The sync channel works both ways; the suspect is the Windows MemexIngest scheduled task. Diagnose on Windows: `schtasks /Query /TN MemexIngest /V` (last run time + result), and whether the task's scan actually covers `%USERPROFILE%\.claude\projects`.
- Cross-device recall re-proven in production during the session: the Mac read the user's July 8-9 Windows chats (BeamNG Amarok mod work, BeamMP server mods) via memex, and this very conversation was indexed + pushed mid-session so the Windows Claude can run the release checklist from it.
- **Linux checklist point 3 VALIDATED LIVE the same evening** (the Linux Claude pulled the runbook from this synced conversation on its next boot, ran it, and pushed the evidence back): `memex stats` 218 conversations, identical to the Mac (chunks 9506 on both), `search "galletas yankees"` returned the Windows-born chat #1, `memex doctor` clean, and Linux's auto-sync had already run unattended on boot (file-sync export 17:10 UTC, HTTP reconcile with the Mac 16:55). Three-store convergence proven. Also verified on the Mac: the peer token printed in that Linux session arrived `[REDACTED]` in the indexed copy (the redaction pipeline held), and the "Projects 0 on Linux" stats difference is not a sync bug (no design_chat has a project link on the Mac either). Still unconfirmed: Linux INBOUND reachability from the Mac (showed offline the whole window; may simply have been shut down). The Mac token was on-screen in that session, so the rotation item now firmly covers all three tokens.
- **Windows checklist points 1-2 VALIDATED LIVE later the same day** (the Windows Claude recovered the checklist from the synced conversation, ran it, and the results were read back on the Mac via memex, the product dogfooding itself): the junk `aaa-junk` snapshot was skipped with a warning while the tick completed (import + export), the truncated own snapshot was rewritten whole (86.2 MB, self-heal), and `memex search "brownie sin TACC"` on Windows returned the Linux-born chat first. Bonus finding: an orphaned pid-suffixed `windows.memexsync.gz.*.tmp` from 2026-07-04 in the shared folder (an interrupted export leaves its temp behind), deleted by hand; a future export could sweep its own stale temps. The MemexIngest diagnosis did NOT run there yet (still open).
- **Token rotation: Mac + Windows DONE the same evening (Linux pending its next boot).** The live rotation surfaced three real UX/robustness gaps, all queued in ROADMAP under `memex token --rotate`: the serve caches the token in RAM and never re-reads the file (Windows authed for hours against a token whose file was gone, then rejected the file's fresh value until a service restart); `sync connect` persists the peer BEFORE validating the token (a failed attempt overwrites a good pairing, hit twice); and the Windows hidden token prompt mangles pastes (`--token "<value>"` avoids it). Two intermediate tokens touched the chat during the process and were rotated again immediately; the final tokens never did. Both directions re-verified (`status --check` ok).
- **Linux token rotation closed the same night** (all three devices now run tokens that never touched a chat; `status --check` green on every pairing), and a **docs audit before closing the release** filled the last three README gaps with the gotchas lived this week: the Fast Startup / NTFS read-only dual-boot trap, the `tailscale ping`-works-but-TCP-times-out TUN-conflict signature (the 2026-07-04 Hamachi find), and a Rotating-a-token how-to (with the restart-first lesson from today's live rotation).
- **Next.** Unchanged from 2026-07-05 (then PR -> tag `v0.5.0` -> publish) minus what closed today; still open: the MemexIngest diagnosis on Windows (minor), then PR. Queued to ROADMAP (user's call, not for this release): the Windows PowerShell SessionEnd hook, a Linux installer-scheduled backstop, `setup` optionally wiring the hook, and `memex token --rotate` + its two related hardenings.

## 2026-07-05: The release becomes 0.5.0; from-zero validation essentially complete

- **Version bump 0.4.4 -> 0.5.0** (user's call): the release carries the whole multi-device install redesign + file-based dual-boot sync + MCP write tools, a feature set that reads as a minor bump, not a patch. `pyproject.toml` + `server.json` (both fields) + `uv.lock`; the CHANGELOG's unpublished 0.4.3 and 0.4.4 sections were MERGED into one `0.5.0 - unreleased` section (on PyPI this release follows 0.4.2 directly, so one section tells the whole story). 0.4.3/0.4.4 remain internal milestone names in DEVLOG/handoff history.
- **From-zero validation, day-2 results (2026-07-04 evening):** the recall circle was proven on ALL THREE OSes with live test chats (Linux "brownie sin TACC", Windows "galletas yankees sin TACC", both indexed there via the MCP write tools and read+summarized from the Mac). Mac<->Windows network sync initially timed out in BOTH directions despite `tailscale ping` working: root cause was a HAMACHI adapter (installed 2026-07-02 for a BeamMP game server, found via the user's own synced chats) breaking Windows' OS traffic through the Tailscale TUN; fixed by disabling Hamachi + restarting the Tailscale service. Diagnostic signature to remember: `tailscale ping` OK + OS ping/TCP timeout both ways = local TUN/routing conflict, NOT memex, NOT the firewall rule. All three stores converged (Linux finishes on its next boot).
- **Publish model clarified for the user:** publishing to PyPI updates nothing by itself; each device upgrades explicitly. The three devices currently run BRANCH installs, so after publish they switch with `uv tool install --force memex-chats` (a plain `uv tool upgrade` would re-pull the git branch). MCP tools update with the local install (restart Claude Code); the Chrome extension is a separate channel.
- **Remaining before the PR:** the Phase-5 robustness spot-checks on Windows (poisoned `aaa-junk` snapshot + own-file self-heal), the `search "brownie sin TACC"` cross-check on Windows, Linux's next-boot convergence check, token rotation (three tokens pasted in chat during validation). Then: PR -> merge -> tag `v0.5.0` -> publish, switch devices to PyPI, republish the MCP registry (server.json already 0.5.0), upload the 0.2.5 extension.

## 2026-07-04: From-zero validation live (Windows + Linux) + installer restart fix (0.4.4)

The user's from-zero runbook started for real (the branch was pushed first: `cbe4fe1` had been local-only and branch installs pull from origin). Windows and Linux reinstalled from the branch; the Mac acts as the network peer and got `setup --sync` (now sync-reachable at its Tailscale address, all three gate flags persisted).

- **Windows firewall auto-open VALIDATED LIVE (the last never-live-tested code path).** Fresh `memex setup` on Windows with the manual rule deleted first: the Firewall row printed "opened inbound TCP 5777 (rule 'Memex sync 5777')" on the first run and "already open" on the second (idempotent). Both outcomes per design. Note: the Windows Phase 0 skipped the data-dir rename, so the persisted flags/token predated the boot (the serve was already sync-reachable at logon); that is also what masked the restart bug below on Windows.
- **[release blocker, found live] `setup --sync` did not take effect until reboot on Linux (and latently on Windows).** Linux from-zero: plain `setup` (serve starts loopback-only), then `setup --sync` printed "reachable on 100.70.96.57:5777", but a `sync connect` from the Mac got CONNECTION REFUSED: the running serve predated the flags, and the serve reads the persisted sync flags only at startup. Root cause: `systemctl --user enable --now` does not restart an active unit; on Windows `schtasks /Run` is ignored while the task is running (`MultipleInstancesPolicy IgnoreNew`) and `/Create /F` replaces the definition without stopping the running instance. macOS was already correct (`launchctl unload` + `load`). Fix: the Linux installer now does `enable` + `restart`; Windows does `/End` then `/Run` for the SERVE task only (the periodic ingest task is never killed mid-run, its next trigger picks up a new definition); same fix in the repo-mode `install-autostart.sh`.
- **Test-isolation hole exposed by the Mac's own opt-in (two symptoms, one root).** Enabling sync for real on the dev Mac broke `TestServeCommand::test_runs_uvicorn_with_defaults` (`serve` with no flag reads `sync_state.json` next to the DB, so it saw the real gate) AND made the suite crawl from ~5 s to stuck-for-minutes: every `TestClient` lifespan started the real auto-sync loop against two offline peers, paying connect timeouts per test. Root fix: a GLOBAL autouse fixture in `tests/conftest.py` (`_isolate_machine_state`) points `settings.db_path` (and everything derived: gate, peers, history, token) at a per-test tmp (the "tests must write a TEMP DB" lesson of 2026-06-28, now enforced mechanically for every test).
- **Linux file sync from-zero: the fresh store repopulated through the folder.** `setup --sync-dir /mnt/windows/memex-sync --device-name linux` pulled **195 conversations from Windows' snapshot** in the initial import (Windows' store had grown past the Mac's ~185 and includes the user's pending Windows terminal chat). Fresh-store repopulation is validated via the FILE transport; the network phase re-runs on Linux's next boot (the fixed installer, or the boot itself, brings the serve up with the flags applied).
- **Validation.** +2 regression tests (Linux install must `restart` and never `--now`; Windows install `/End`s only the serve task), 683 green, ruff + format + core mypy clean, `sh -n` on the edited script.
- **Next.** Finish the runbook: Windows file-sync phase + the robustness spot-checks; verify Linux reachable after its next boot; converge all three stores; then PR -> tag -> publish 0.4.4 (release target Friday 2026-07-10). Rotate the tokens pasted in chat during the validation (Mac + Linux + Windows) and re-pair.

## 2026-07-02: Malformed-input robustness review of sync + redaction (0.4.4)

A focused review of the sync and redaction paths against malformed input (ReDoS, unbounded quantifiers, decompression limits, giant lines, per-file isolation), requested by the user as a follow-up to the 2026-07-01 audit. Every suspicion was verified empirically against the real code before being called a finding. Still 0.4.4, branch `feature/install-sync-reachable`.

- **Redaction: clean.** Timed adversarial single-line blobs against every rule family: the worst case found was the JWT rule on an `eyJ`-repeat blob at 0.27 s / 512 KB, scaling linearly (64/160/512 KB in proportion). The 2026-07-01 ReDoS fixes hold (`otpauth://` x16k in 0.09 s, `x-` x80k in 0.12 s, packed BEGIN markers in 0.06 s). The few literally-unbounded quantifiers left (`\s+` in bearer/assignment/labeled, `=*`, `{3,}` in the dotted-run rule) each abut a disjoint character class, so they are linear (measured, not assumed); no changes.
- **[medium] File-sync per-file isolation had a too-narrow exception tuple.** The per-file guard in `import_once` and the catch in `read_header` listed `(OSError, ValueError, RecursionError)`, but a TRUNCATED gzip raises `EOFError`, a bit-flipped deflate stream raises `zlib.error`, and a crafted manifest whose uuid is a list raised `TypeError` from the diff; none subclass those. Reproduced all three: one bad peer file aborted the whole import loop (healthy peers silently stopped converging), `sync_once` never reached the self-export, and a truncated OWN snapshot made `export_snapshot` crash forever (it reads its own header for the fingerprint skip-check). Fix: both guards now catch `Exception` (same policy the per-record insert already used), so a bad file skips only itself and a corrupt own file reads as absent and is REWRITTEN (self-heal). `records._by_uuid` additionally requires a string uuid, hardening the diff on both transports.
- **[medium] Record lines are parsed whole too, so the 256 MB line cap allowed a ~6 GB parse.** The 2026-07-01 header fix reasoned record lines could keep a generous cap because they are "streamed"; but each record line is still `json.loads`'d whole before the uuid filter, and measured inflation of container-heavy JSON is ~24x, so a tiny crafted gzip could make the always-on serve materialize ~6 GB at the 256 MB cap. Lowered `_MAX_LINE_BYTES` to 96 MB: still well above the largest legit record (~57 MB at the 5000-chunk cap), worst-case transient now ~2.3 GB (a bound, not a closure; noted as a residual in the internal notes).
- **Validation.** +6 regression tests (truncated gzip, bit-flipped gzip, non-string manifest uuid at both the file and records layers, sync_once-still-exports-past-a-bad-file, export-self-heals-own-truncated-snapshot), **681 green**, ruff + format + core mypy clean. Re-ran the original repro probes: every case that previously raised now returns a summary and the healthy peer imports.
- **Next.** Committed locally, NOT pushed (user's call, keep it local for now). What remains before publish is live validation, not code: the full from-zero install/setup/sync runbook for Windows + Linux is in `handoff.md` ("From-zero validation runbook"), including the one never-live-tested piece (the Windows firewall auto-open in `setup --sync`) and a live spot-check of this round's self-heal fix.
...
- **MCP write tools VALIDATED LIVE (full circle).** After the user reinstalled 0.4.4 on Windows, a NEW Claude Code session there was asked "syncea esta conversacion"; Claude called `mcp__memex__index_terminal_sessions` + `sync_now` (visible in the transcript), and the Mac then pulled that conversation (`sync now` -> pulled 1) and read it back. This morning the Windows Claude could NOT sync (read-only tools only); by end of day it did, via the tools built the same session. Bi-directional cross-device recall proven both ways (Mac->Windows and Windows->Mac).
- **Next.** Live-validate the firewall auto-open in `setup --sync` on Windows (the code is untested; a manual persistent port rule is already in place); restart Claude Code on the Mac to expose the new tools there; then the PR (deferred by the user until publish) -> tag/publish 0.4.4.
...
- **Key reuse: the sync core is already transport-agnostic.** `records.select_reconcile` (last-writer-wins diff), `records.serialize_conversation` / `insert_record` (wire format + consistent insert), and `records.local_manifest` know nothing about HTTP. Only the `client.py` fetch/push functions are HTTP-specific. So file sync is a new transport over the SAME core, not a parallel implementation: a file record and an HTTP record are byte-identical and cannot drift.
- **Design.** New `sync/file_sync.py`: each device exports a gzip JSON-lines snapshot of its full store to `<sync_dir>/<slug(device)>.memexsync.gz` (a header line with the manifest + fingerprint, then one serialized record per line), and imports every OTHER `*.memexsync.gz` in the folder. A device only ever writes its own file and reads the others', so they never clash (and on a dual-boot only one OS runs at a time anyway). Sync is pull-only against each peer file; the "push" half is emergent (my snapshot is my full state, the peer pulls my newer conversations on its next boot). Converges by eventual consistency over two boots. A same-`updated_at` fork is left untouched and reported, exactly like the network mode.
- **No new daemon / no service change.** It rides the existing serve auto-sync loop: `_auto_sync_once` now runs the file sync after the network-peer loop (even with zero network peers), under the same non-blocking ingest lock, and the lifespan starts the loop when a `sync_dir` is configured. So the autostart `memex serve` picks it up with no per-OS template change (same lesson as 0.4.3's `serve_sync`: a persisted flag the running process reads, not N service edits).
- **Config.** Two new gate fields in `sync_state.json` (`sync_dir`, `device_name`), set by `memex setup --sync-dir <folder> [--device-name <name>]` (creates the folder, persists, enables the gate + auto-sync, does an initial export/import, prints the one command for the other OS). `MEMEX_SYNC_DIR` / `MEMEX_DEVICE_NAME` env overrides for tests/advanced. New `memex sync file-sync` (manual immediate file sync) and `memex sync now` now folds in file sync; `memex sync status` shows the folder/device/last-sync. `set_gate` grew an `_UNSET` sentinel so writing the file config preserves the booleans and vice versa.
- **Dual-boot asymmetry (documented).** The shared folder must live on the Windows/NTFS partition: Linux reads/writes NTFS out of the box, Windows cannot read ext4. Setup prints this. For two separate rarely-online machines, any shared location works (USB, cloud folder).
- **Security.** A snapshot file is attacker-shaped input (same trust as a paired peer): `insert_record` re-asserts the chunk cap + list guards + dim check, and the reader bounds each decompressed line (a flat 256 MB anti-gzip-bomb guard, above the largest legit single-conversation record but far below "inflate a tiny gzip into GBs"). `import_once` is resilient per file (a read error on one snapshot aborts only that file, logged, never the whole sync). No re-redaction (vectors already encode the text, same as network).
- **Self-review caught two of my own bugs before tests:** (1) I had tied the per-line cap to the 16 MB network body cap, which would falsely reject a legit large record (up to ~57 MB at 5000 chunks); fixed to a flat 256 MB guard. (2) a `ValueError` from the bounded line reader inside `_iter_records` was not caught in `import_once` and would crash the import; wrapped per file.
- **Validation.** 30 new tests: 24 in `test_file_sync.py` (naming/header, export skip-if-unchanged, B-imports-A, newer-wins/older-not-overwritten, fork-left-untouched, would-push reporting, two-pass convergence, three-device, model/dim mismatch refused, does-not-import-own-file, corrupt/oversized/malformed input, chunk-cap enforced on the file path, dir/device resolution precedence) + 6 in `test_sync.py` (file-sync state round-trips + flag preservation, file sync runs with no network peers, skipped while the ingest lock is busy). **649 tests green**, ruff + format + core mypy clean, `uv build` + twine pass. CLI smoke: `memex setup --sync-dir` end to end on isolated temp DBs (folder created, gate persisted with `serve_sync` untouched, two devices each export + read the other's snapshot, `status` shows it).
- **Auto-sync-after-capture (user-chosen, same session).** Addressed the UX question about `memex sync now`: clarified the model (sync is whole-store, terminal-only, does not read claude.ai; the extension captures, then sync propagates; "several open tabs" is a non-issue because sync is not per-conversation), and the user picked "auto-sync after each capture (debounced)". Implemented: the `/ingest/conversation` endpoint debounces a sync after a successful ingest (`_schedule_post_capture_sync` cancels any pending asyncio timer and starts a fresh one; `_debounced_post_capture_sync` sleeps the window then runs `_auto_sync_once` off the loop). A capture burst coalesces into ONE sync (each capture resets the timer, so it fires once the burst settles); fire-and-forget (never blocks/fails the capture); gated on sync-enabled + a target (peer or file dir) + `MEMEX_SYNC_ON_CAPTURE` (default on); window `MEMEX_SYNC_ON_CAPTURE_DEBOUNCE_SECONDS` (default 10); covers network + file modes; cancelled on lifespan shutdown. For dual-boot the marginal value is small (the other OS reads on its next boot), but it makes always-on multi-device near-real-time. 7 new tests. **656 green.**
- **Next.** The PR, tag/publish 0.4.4; live-validate file sync on the real dual-boot.
...
- **Live install validated on Windows.** Installed 0.4.3 from the branch via `uv tool install git+...@feature/install-sync-reachable`. `memex setup --sync` came up perfect (Sync OK reachable on the Tailscale IP, both Scheduled Tasks, connect line). VALIDATED CROSS-DEVICE: from the Mac, `GET /sync/manifest` on Windows' AUTOSTART serve (`100.75.209.56:5777`) returned HTTP 200 with the token — proving the whole persisted chain end to end from another device (task -> pythonw serve -> read serve_sync -> bind 0.0.0.0 -> allow-list the Tailscale IP -> token gate). Gotcha (not our bug): the entrypoint copy to `~/.local/bin/memex-mcp.exe` failed with "Acceso denegado (os error 5)" — first because a running `memex-mcp.exe` (Claude Code's MCP child) locked it, then because Windows Defender real-time scan blocked writing the new .exe. Fix: close Claude Code + kill memex procs, add a Defender exclusion for `%USERPROFILE%\.local\bin` and `%APPDATA%\uv`.
- **Set-and-forget auto-sync + `memex sync now` (this session, user-requested).** `setup --sync` was reachable-only; cross-device stayed manual unless `MEMEX_SYNC_AUTO=true`. Added a persisted `sync_auto` gate flag (mirror of `serve_sync`); `setup --sync` now turns it on (and `--no-sync` off), and the serve lifespan starts the auto-sync loop if EITHER the persisted flag OR the env var is set. Added `memex sync now` (on-demand two-way reconcile with every peer, same as `reconcile`) for syncing between the 15-min ticks. Documented the capture/real-time model honestly: same-machine is near-instant; cross-device is overlap sync (every 15 min while both online, or `sync now`), never a live relay; the terminal can't read claude.ai (the extension captures on open/reload). New README "Using Memex (after setup)" section with the 4 MCP tools + a real-time latency table. 619 tests green.
