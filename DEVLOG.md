...
**Capture server idle-release (same session, user then reported the capture process at 1.79 GB):** root cause is structural, not the batch size. `memex serve` embeds each captured chat in-process and is always-on (launchd KeepAlive), so once it embedded one chat it held the model + arena (~1+ GB) resident forever. Added an idle-release timer (`MEMEX_INGEST_IDLE_RELEASE_SECONDS`, default 60s): the `/ingest` handler re-arms an `asyncio` `call_later` after each capture; when captures stop, `_release_embedder()` drops `_embedder` and `gc.collect()`s, returning the server to its ~0.15 GB baseline. The next capture reloads lazily (~1-2s, weights cached on disk). Kept hot during a browsing burst (captures closer than the window). 0 disables it (old behavior). Requests serialize through the sync embed on the event loop, so releasing at the end of the handler is safe. 4 new tests. Same fix is NOT needed for the MCP server (also embeds on search, but a search embeds one query so its footprint is small; left resident for search latency). The transient `ingest-claude-code` process already exits and frees RAM, so the batch-size change covers it.
