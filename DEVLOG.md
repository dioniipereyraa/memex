...
- **Next.** Unchanged from 2026-07-05 (then PR -> tag `v0.5.0` -> publish) minus what closed today; still open: Linux token rotation on its next boot, the MemexIngest diagnosis on Windows (minor), commit + PR. Queued to ROADMAP (user's call, not for this release): the Windows PowerShell SessionEnd hook, a Linux installer-scheduled backstop, `setup` optionally wiring the hook, and `memex token --rotate` + its two related hardenings.
