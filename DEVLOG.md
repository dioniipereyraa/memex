...
Gotcha logged: `launchctl kickstart -k` on `com.memex.serve` mid-backfill SIGTERMs (-15) the in-flight ingest subprocesses, so a handful of chats showed up as failed in the extension. No data loss (the killed ones were re-ingests of already-stored chats; the incremental confirmed "0 to fetch"), and a missing chat self-heals on the next backfill. Lesson: do not restart `memex serve` during a bulk ingest. Next: M5 (README onboarding rewrite, repackage the extension, fresh-machine e2e) + Phase B of the install.
