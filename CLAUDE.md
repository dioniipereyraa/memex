...
- **There is no un-observable channel between a content script's MAIN and ISOLATED worlds.** `inject.js` runs in claude.ai's MAIN world, same as a page-world attacker (claude.ai XSS). A nonce handshake to prove a `postMessage` came from `inject.js` does not work: the nonce rides postMessage and is observable, so an adaptive attacker replays it. Do NOT ship a nonce as if it were a security boundary. Validate structure/origin, document the limit, and rely on the server treating all captured content as untrusted (envelope + control-char strip). The vector is index poisoning (integrity), not exfiltration, and needs a claude.ai compromise.
- **Unauthenticated DCR `/register` is unbounded (upstream FastMCP).** The OAuth-proxy client store `put()` passes no `ttl` (transactions/codes/tokens all expire; client registrations do not), so an anonymous flood writes unbounded encrypted client files → disk-exhaustion DoS. DCR is necessarily pre-auth. No data exposure. Mitigate at the tunnel (rate-limit `POST /register`); do NOT age-prune the client store locally (you cannot tell the live claude.ai client from junk by mtime, and pruning it forces a user re-auth). Real fix is a TTL upstream.
