...
- [ ] **(M3)** Incremental: skip conversations already indexed/unchanged before fetching (a server `GET /ingest/known` returning `{uuid: updated_at}`, compared against the list's `updated_at`), so re-runs only pull what changed. Add a `provenance` tag (backfill vs live vs export). Today every run re-fetches all 94 and leans on server-side dedup; cheap at 94, wasteful at thousands.
- [ ] **(M4)** Popup "Backfill history" button + progress UI (N/total, errors) + resumability (persist a cursor in `chrome.storage` so a closed tab resumes). Needs a progress channel: content.js currently drops non-`json-*` message kinds, so add `backfill-progress` to `KNOWN_KINDS` + background handling.
