...
### Known limitation
- **Unauthenticated DCR registration is unbounded (availability, no data exposure).** The remote connector's dynamic client registration (`POST /register`, necessarily pre-auth so claude.ai can register) persists one small encrypted file per call with no TTL or cap in the upstream FastMCP OAuth proxy, so a sustained flood could exhaust disk. No chat data is exposed. Mitigate at the tunnel layer (rate-limit / restrict `POST /register`); a proper fix (TTL on the client store) belongs upstream. Tracked.
