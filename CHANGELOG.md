...
- **Set-and-forget auto-sync.** `memex setup --sync` now also turns on **auto-sync** (a new persisted `sync_auto` gate flag, mirror of `serve_sync`), so paired devices reconcile by themselves every `MEMEX_SYNC_INTERVAL_SECONDS` (default 900) while both are online, with no env var. The serve lifespan starts the loop if either the persisted flag or the `MEMEX_SYNC_AUTO` env var is set. `memex setup --no-sync` turns it off with the rest.
- **`memex sync now`.** An on-demand two-way reconcile with every paired device, for when you do not want to wait for the auto-sync tick (e.g. to push the conversation you are having right now). Same as `reconcile`; it propagates what is already indexed locally (a claude.ai chat is captured by the extension on open/reload).
...
- **Auto-sync right after a capture.** When the capture server indexes a chat (the Chrome extension capturing a claude.ai conversation), it now debounces a sync so the new conversation reaches your other devices within seconds instead of waiting up to `MEMEX_SYNC_INTERVAL_SECONDS`. A burst of captures (a backfill, several open tabs) coalesces into ONE sync once it settles; it only runs while sync is enabled with a target (a paired peer or a file-sync folder), and never blocks or fails the capture. It covers both modes (network reconcile + file sync). Off-switch `MEMEX_SYNC_ON_CAPTURE=false`; the coalescing window is `MEMEX_SYNC_ON_CAPTURE_DEBOUNCE_SECONDS` (default 10).
- **Ask Claude to index and sync (two new local MCP tools).** The `memex-mcp` stdio server (the one Claude Code uses) now also exposes `index_terminal_sessions` (runs the `ingest-claude-code` scan so the current terminal session becomes searchable) and `sync_now` (an immediate reconcile with paired devices + a file sync). So from Claude Code you can say "index this session" or "sync my devices" and Claude does it, instead of dropping to a terminal. Both are LOCAL-ONLY: they are never registered on the remote claude.ai connector, since they mutate the store and reach the network. `index_terminal_sessions` shares the single-flight ingest lock; `sync_now` requires sync enabled and reports per-device pulled/pushed/forks.
- **Windows: open the sync firewall port automatically.** In sync-reachable mode the always-on serve binds `0.0.0.0:5777`, but Windows Firewall silently drops inbound connections (a peer just times out) until a rule allows the port. `memex setup --sync` on Windows now adds that inbound rule (idempotent), so a paired device can actually reach it after a plain install. If setup is not elevated it prints the exact one-time admin command instead of failing. (No firewall change is needed on macOS/Linux, or for the dual-boot file-sync mode, which uses no network.)

### Changed
- **README: how a Claude Code session gets indexed.** The terminal-sessions section now leads with the three user-facing triggers (ask Claude via `index_terminal_sessions`, close the conversation with the SessionEnd hook installed, or run `memex ingest-claude-code`) plus the 15-minute backstop, and states the per-OS reality: `memex setup` schedules the backstop on macOS and Windows, Linux wires it by hand, and the SessionEnd hook script has no Windows (PowerShell) equivalent yet.
- **README: the multi-OS gotchas learned in live validation.** The dual-boot section warns that Windows Fast Startup leaves the NTFS partition read-only for Linux (file sync's export fails until Windows shuts down fully); Troubleshooting gains the "`tailscale ping` works but sync times out both ways" signature (a local TUN/routing conflict from another VPN-style adapter, e.g. Hamachi, not memex or the firewall); and the sync section gains a **Rotating a token** how-to (delete the file, RESTART the serve, only then read `memex token` and re-pair the dialing peers; the running server caches the token in memory).
...
### Fixed
- **`memex sync status` now reports the auto-sync state the serve actually uses.** It rendered only the `MEMEX_SYNC_AUTO` env var, so after a `memex setup --sync` (which persists the choice in the gate file precisely so no env var is needed) status said "Auto-sync: off" while the serve's loop was in fact configured to run. The lifespan and status now share one helper (`sync_state.auto_sync_effective`: the env flag, the persisted flag, or a configured file-sync folder), so the report can never drift from the behavior again.
- **Opting into sync now takes effect immediately on Linux and Windows.** The always-on serve reads the persisted sync flags only at startup, but the Linux installer used `systemctl --user enable --now` (which does not restart an already-running unit) and on Windows `schtasks /Run` is ignored while the task is still running, so running `memex setup --sync` AFTER a plain `memex setup` left the old loopback-only server up: another device got "connection refused" until the next reboot, even though setup printed the sync address. Found live in the 0.4.4 from-zero validation. The installers now restart the running service (Linux: `systemctl --user restart`; Windows: `schtasks /End` before `/Run`, for the serve task only, so a periodic ingest run is never killed mid-flight). macOS already restarted (`launchctl unload` + `load`).
...
- **Broad security audit (multi-agent, adversarially verified).** A fresh audit of the code added since the last red-team rounds (file sync, the Windows firewall auto-open, the post-capture sync, the MCP write tools, the auto-sync loop) plus a sweep of the classic surfaces. Five findings confirmed and fixed; no chat data was exposed by any of them (all are availability/robustness hardening on already-trusted boundaries). Fixes:
  - **File-sync decompression hardening.** The single 256 MB per-line cap was the only anti-gzip-bomb guard; a tiny crafted `*.memexsync.gz` could still OOM the always-on server because the header line (the manifest) is parsed whole into memory and a 256 MB decompressed header inflates to gigabytes of Python objects. Added a tight header-line cap (32 MB, still ~140k conversations), a running total-decompressed-bytes budget for the whole snapshot, and made `read_header` catch a `RecursionError` from an absurdly nested header (fails soft to "skip this file").
  - **File-sync per-file resilience.** A poisoned snapshot whose name sorts first could abort the import of every other peer file in that tick (its `read_header` / diff ran outside the per-file guard), silently stalling a healthy device's convergence. The whole per-file body is now wrapped so one bad snapshot skips only itself.
  - **Two redaction ReDoS fixes.** The `otpauth://…secret=` rule and the assignment rule's `x-…`-header branch each had one unbounded lazy quantifier (the lone exceptions to the bounded-quantifier invariant), quadratic on a crafted whitespace-free blob reaching the `claude_code` ingest path (~12 s and ~8 s at ~160 KB / 32 KB). Both bounded; verified linear and still matching real secrets.
  - **`list_recent_chats` response bound.** It emitted each summary verbatim, so up to 100 long (2-3k char) summaries in one response could blow the MCP client's token cap; now truncated to 500 chars like `search_chats` / `find_related`.
- **Malformed-input robustness round (follow-up review).** The sync and redaction paths were re-reviewed against corrupt and hostile input (ReDoS, decompression bombs, giant lines, per-file isolation); redaction came out clean (every pattern measured linear on adversarial blobs). Two file-sync fixes:
  - **Per-file isolation now survives any corruption.** A truncated snapshot (a partial copy on a cloud-synced folder or USB stick, a power cut) raises `EOFError`, mid-stream corruption raises `zlib.error`, and a crafted manifest could raise `TypeError`; none were caught by the per-file guard, so one bad peer file aborted the whole import (healthy peers stopped converging and the device's own snapshot stopped exporting), and a truncated own snapshot made every later export crash. The guards now catch everything: a bad file skips only itself, and a corrupt own snapshot is rewritten on the next sync (self-heal). Manifest entries whose uuid is not a string are ignored by the diff.
  - **Record-line decompression cap lowered (256 MB to 96 MB).** Each record line is parsed whole (one at a time), and adversarial container-heavy JSON inflates ~24x into Python objects, so a tiny crafted gzip could make the server transiently materialize ~6 GB at the old cap. 96 MB still admits the largest legitimate single-conversation record (~57 MB at the 5000-chunk cap) with headroom.
